...
public class UserDaoImpl implements UserDao {
    ...
    static {
        ...
        for (int i = 0; i < length; i++) {
            ...
            users.add(new User((long) (i + 1), "user" + (i + 1), "" + passwords[i], LocalDate.of(years[i], months[i], days[i]), Instant.now()));
        }
        // 为了方便测试登录功能，添加的名称密码固定的用户
        users.add(new User(0L, "user000", "123456", LocalDate.now(), Instant.now()));
        // 控制台输出所有数据
